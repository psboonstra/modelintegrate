...
author: "Step 2: Processing the results and creating figures/tables"
date: ""
...
If you haven't read through `run_sims.html`, which is Step 1, do so first. 
...
`sims_per_scenario` can be any number; the object is required but the specific
value is not used. `which_batch` is also not used but it still must be one of
the batch numbers. `total_num_arrays` should be the total of all arrays across
all batches. For each of `i` = 1, ..., `total_num_arrays`, there should exist
three files:

1. `paste0("out/job",i,"_scores.csv")`
2. `paste0("out/job",i,"_betas.csv")`
3. `paste0("out/job",i,"_array_stats.csv")`

All of these are output by `run_sims.R`
...
Now source the helper functions
...
We can try to read in all of the expected files (all `total_num_arrays` of them)
by specifying `read_all_sims = TRUE` or we can choose a random subset of 50 of
them to save some time. If there are any arrays that did not finish to
completion, they will be kept track of in `not_found`. 
...
### Assess complexity weights

This next step isn't directly relevant to assessing the performance of the methods. 
`scenario_complexity_weight` is an adhoc number we created for each scenario
that tries to quantify in relative terms how long it takes for all methods to
analyze a single dataset and therefore allocate more arrays to higher complexity jobs.
Below, we compare the current value of `scenario_complexity_weight` against `total_runtime_secs`
to see how well we did. We also fit a linear regression model to estimate
how the factors in each scenario drive the runtime. If we re-run the simulation 
study, we can use this information to readjust the weights. 
...
Now we are ready to make the tables and figures in the manuscript and supplement. 
We first delineate which methods are primary and which are secondary (recall that
some of the methods we fit in `run_sims.R` were not included at all in the 
manuscript). 
...
We also check that there are no replicated values of `data_seed` within
any of the 64 scenarios. The value of the expression below should be `TRUE` if there are no replicated values. 
...
Now the only thing left to do is make the figures and tables. 
