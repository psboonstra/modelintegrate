...
assigned to one of the 96 scenarios presented in the manuscript. More arrays
...
You now have an object `all_scenarios` with 96 rows, which is the number of
...
This identifies the tuning parameter to use for the adaptive Bayesian updates (SAB, SAB2, SAB2(Ext)). It uses stochastic simulation to approximate an integral. It
...
Finally, we set up the results to be stored. 
```{r}
i = 1
...
with an * are not reported in the manuscript):
    ...
    i. SAB (`sab`)
    j. SAB2 (`sab2`)
    k. Alternative formulation of SAB2 where $\omega^2$ does not scale the variance (`sab2_alt`)*
    l. SAB2(Ext) (`sab2_flexible`)
