---
title: "Reproducing the results in Boonstra and Orozco del Pino (2023)"
author: "Step 1: Running the simulation study"
date: ""
output: html_document
---

```{r setup, include = TRUE, echo = FALSE, purl = FALSE}
...
`convergence_tol` can be increased for purposes of testing methods but was set
to `1e-4` in the actual simulation study. Similarly, you can skip the Bayesian
methods, which tend to take the longest. Obviously don't do this if you want
to run the Bayesian methods. 
...
Each batch consisted of 999 arrays, as indicated by `total_num_arrays_by_batch`,
with each array assumed to be sent to a different compute node. Each array was
assigned to one of the 64 scenarios presented in the manuscript. More arrays
were devoted to scenarios that took longer to run (this is handled in
`sim_functions/scenario_setup.R`). Within a batch, the total number of
independent datasets generated and analyzed by each method is given by
`sims_per_scenario_by_batch`, and the arrays that were devoted to a particular
scenario were assigned an equal number of datasets to generate.
...
Now choose the generating parameters for the simulation study and source the 
helper functions.
...
You now have an object `all_scenarios` with 64 rows, which is the number of
possible combinations of `n_curr`, `n_hist`, `scenario_name`,
`different_external_model`, and `different_covariate_dist.` Each row spans one
or more values of `array_id`, and we identify the row corresponding to our
current value of `array_id`. 
...
`array_id_with_offset` accounts for previous batches. 
...
Now source the code for implementing each method. 
...
The total number of unique seeds needed across all arrays is equal to
`sims_per_scenario`. `which_seeds` is a logical vector with length equal to
`sims_per_scenario`. The elements that are `TRUE` correspond to the individual
simulated datasets that this `array_id` will be responsible for. This ensures
that the set of seeds are identical across scenarios and distinct within a
scenario.

Having done that, we generate all of the seeds for our batch and then keep the
seeds we need for this array. 
...
Access the true effect sizes and other relevant information for governing
...
This identifies the tuning parameter to use for the adaptive Bayesian updates
(SAB, SAB2, SAB2(Ext)). It uses stochastic simulation to approximate an
integral. It only needs to be run once.
...
Finally, we set up the results to be stored. `all_scores` keeps track of performance
in estimating the betas and predicting outcomes; `all_beta_ests` keeps track
of the individual estimates of the regression coefficients; `all_lambda_ests`
keeps track of the estimates of the lagrangian terms (this only applies to the
constrained MLE approaches); and `all_bayesian_diag` keeps track of MCMC/HMC-specific
diagnostics (this only applies to the SAB-type approaches).
...
Here is where the actual simulation occurs. We just loop through the number of
simulated datasets to be generated and analyzed, keeping track of our results
along the way. The basic steps of the loop are:

1. Generate data for both the historical and the current study
2. Analyze the historical study to get your $\tilde\theta$
3. Estimate the coefficients and performance for the following methods (those
with an * are not reported in the manuscript):

a. The true values of the coefficients (`truth`)*
b. GLM(MLE) (`glm_vanilla`)*
c. GLM(Bayes) (`glm_bayes`, the reference method)
d. Ratios (`ratios`)
e. CML(SP) (`cml_saddlepoint`)
f. CML(NR) (`cml_newtonraphson`)
g. CML(NR) with a much smaller step size (`cml_newtonraphson_step`)* 
h. GIM(Sand), saddlepoint, sandwich variance estimate (`gim_sandwich`)
i. GIM(Aut), as implemented in `gim` R package (`gim_author`)
j. GENMETA, as implemented in `GENMETA` R package (`genmeta_author`)
k. SAB (`sab`)
l. SAB2 (`sab2`)
m. Alternative formulation of SAB2 where $\omega^2$ does not scale the variance (`sab2_alt`)*
n. SAB2(Ext) (`sab2_flexible`)

4. Write the separate .csv files for performance (squared error, brier score, convergence, etc), 
coefficient estimates, and running time. 
